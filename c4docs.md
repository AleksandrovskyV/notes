...
Честно отвечу, что не знаю о них ничего кроме факта: без них вызов метода на выполнится,<br>так как писатель посчитал вынести их обязательными на вызове, добавив в сигнатуру (шапку) функции.<br>А также что идут они в относительно строгом `друг за другом` порядке после открывающей скобки(<br>

И в конце примера идёт именной/необязательный аргумент: <strong>name="кнопка"</strong>,<br>который отличается от первых двух наличием <strong>"="</strong>, что говорит о его происхождении.<br>Кстати именные могут стоять в разном относительно `друг от друга` порядке и попросту отсутствовать. <br>Такие вот они, неопределённые типы...<br><br><strong>AI:</strong> Термин "тип" был взят тобой опромечтиво к такому типу абзаца... <br>
