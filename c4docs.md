...
То честно отвечу, что знаю лишь факт: без их записи метод на выполнится,<br>так как писатель посчитал нужным вынести их обязательными при вызове, добавив в шапку (сигнатуру).<br>Идут они в строгом `друг за другом` порядке после открывающей скобки(<br>

В конце примера красуется именной и необязательный аргумент вида: <strong>name="кнопка"</strong>,<br>что отличается от первых двух наличием <strong>"="</strong>, говорящим о его происхождении.<br>Внезапно, именные могут стоять в разном относительно `друг от друга` порядке и попросту отсутствовать. <br>Такие вот неопределённые они, типы...<br><br><br><strong>AI:</strong> Термин "тип" был взят тобой опрометчиво к такому типу абзаца... <br>
