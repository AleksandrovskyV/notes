...
То честно отвечу, что знаю лишь факт, что без их записи метод не выполнится,<br>так как писатель вынес их в шапку обязательными.<br>Следуют в строгом `друг за другом` порядке после открывающей скобки(<br>

В конце примера красуется именной и необязательный аргумент вида: <strong>name="кнопка"</strong>,<br>что отличается от первых двух наличием <strong>"="</strong>.Внезапно, именные могут стоять в разном <br>относительно `друг от друга` порядке и попросту отсутствовать.<br>Такие вот неопределённые они, типы...<br><br><br><strong>AI:</strong> Термин "тип" был взят тобой опрометчиво к такому типу абзаца... <br>
