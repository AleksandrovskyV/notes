...
Честно отвечу, что не знаю о них ничего, кроме факта: без них метод на выполнится,<br>так как писатель посчитал вынести их обязательными на вызове. А также что,<br>идут они в строгом порядке - друг за другом... <br>

И в конце примера идёт именной/необязательный аргумент: <strong>name="кнопка"</strong>,<br>который отличается от первых двух наличием "=", говорящим о его происхождении. Именные могут стоят в разном порядке и попросту отсутствовать на вызове. <br>Такие вот неопределённые типы...
